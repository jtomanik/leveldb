...
use super::Database;
use leveldb_sys::{
    leveldb_create_iterator, leveldb_iter_destroy, leveldb_iter_key, leveldb_iter_next, leveldb_iter_seek, leveldb_iter_seek_to_first, leveldb_iter_seek_to_last, leveldb_iter_valid,
...
pub trait LevelDBIterator<'a, K: Key> {
    ...
    fn seek(&self, key: &K) {
        unsafe {
            key.as_slice(|k| {
                leveldb_iter_seek(self.raw_iterator(), k.as_ptr() as *mut c_char, k.len() as size_t);
            })
        }
    }
}
