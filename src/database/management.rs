...
pub fn destroy(name: &Path, options: Options) -> Result<(), Error> {
    ...
    unsafe {
        ...
        leveldb_destroy_db(c_options, c_string.as_bytes_with_nul().as_ptr() as *const libc::c_char, &mut error);
...
pub fn repair(name: &Path, options: Options) -> Result<(), Error> {
    ...
    unsafe {
        let c_string = CString::new(name.to_str().unwrap()).unwrap();
        let c_options = c_options(&options, None);
        leveldb_repair_db(c_options, c_string.as_bytes_with_nul().as_ptr() as *const libc::c_char, &mut error);
